...
# 24/09/19
#   * STUFF ADDED BY BEN
#   * Ability to set VLAN ID 
#   * JSON configuration file support
#   * Check DNS and add if missing
#   * Split Nested ESXi Root password from VCSA Root password
#   * Added statement to deploy non VSAN cluster if JSON file is set not to build vSAN
...
$ESXi65OfflineBundle = $DeployConfig.FilePaths.ESXi65aOfflineBundle # Used for offline upgrade only
...
$offlineUpgrade = $upgradeESXiTo65a = $DeployConfig.Misc.UpgradeESXiTo65a
...
if($confirmDeployment -eq 1) {
    ...
    Write-Host -ForegroundColor Yellow "`n---- VCSA Configuration ----"
     Write-Host -NoNewline -ForegroundColor Green "VCSA Network: "
...
if($deployVCSA -eq 1) {
    if($DeploymentTarget -eq "ESXI") {
        ...
        $config.new_vcsa.esxi.deployment_network = $VCSANetwork
        ...
        $config.new_vcsa.vc.deployment_network = $VCSANetwork
...
if($setupNewVC -eq 1) {
    ...
    if($configureVSANDiskGroups -eq 1) {
        ...
        #Trying to amend the existing cluster to enable vsan from line 903
